...
export default function Resetpsw(props) {
  ...
  const alluser = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/auth/users`,
      {
        method: "GET",
      }
    );
  ...
  const resetpassword = async (e) => {
    e.preventDefault();
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/auth/resetpassword/${id}`,
      {
        method: "PUT",
        headers: {
    ...
    navigate("/login");
    props.alert(
      "Password has been Changed Please Login to continue",
      "warning"
    );
  };
      ...
      <div className="row justify-content-center">
        <div className="col-md-6">
          ...
          >
            ...
            <button
              type="submit"
              disabled={detail.password !== detail.cpassword}
