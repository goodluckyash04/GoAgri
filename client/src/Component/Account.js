...
export default function Account(props) {
  ...
  const { cartList, fetchcart, deletecartorder, oDetail, fetchorderlist } =
    cart;
  ...
  const accountDetail = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/auth/getuser`,
      {
        method: "GET",
        headers: {
  ...
  const userdelete = async (id) => {
    ...
    if (x === "y" || x === "Y") {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER}/api/auth/deleteuser/${id}`,
        {
          method: "DELETE",
        }
      );
    ...
    <div className="container" style={{ minHeight: "70vh" }}>
      ...
      <div className="row justify-content-between">
        <div className="mb-5 col-3">
          ...
          <EditAccount
            detail={details}
            acdetail={accountDetail}
            alert={props.alert}
          />
        </div>
        <Link className="col-2 text-success" to="/resetpassword">
          Reset Password
        </Link>
        <button
          className="btn btn-success mb-5 me-2  col-3"
          onClick={() => {
            userdelete(id);
          }}
        >
      ...
      <div className=" mt-3 p-3">
        ...
        <div className="row">
          <div className="col-1">
            <h4>
              <b className="text-success">Sr.</b>
            </h4>
          </div>
          <div className="col-2">
            <h4>
              <b className="text-success">Date</b>
            </h4>
          </div>
          <div className="col-6">
            <h4>
              <b className="text-success">OrderId</b>
            </h4>
          </div>
          <div className="col-3">
            <h4>
        ...
        {oDetail &&
          oDetail.map((o, i) => {
            return (
              <div className="row mt-2 fs-5" key={i}>
